  ...
  try {
    ...
    try {
      ...
      return res.json({ 
        success: true, 
        drivers: [],
        message: 'Error checking drivers table',
    ...
    if (!tableCheck.rows[0].exists) {
      ...
      return res.json({ 
        success: true, 
        drivers: [],
        message: 'Drivers table does not exist. Run: node database/setup-drivers.js'
      });
    }
    
    console.log('✅ Drivers table exists, fetching drivers...');
    
    const result = await query(
      'SELECT id, name, contact FROM drivers WHERE active = true ORDER BY name'
    );
    
    console.log(`✅ Fetched ${result.rows.length} active drivers`);
    
    if (result.rows.length === 0) {
      console.warn('⚠️  No active drivers found in database');
      return res.json({ 
        success: true, 
        drivers: [],
        message: 'No active drivers found. Run: node database/setup-drivers.js'
      });
    }
...
router.get('/all', async (req, res) => {
  try {
    const result = await query(
      'SELECT id, name, contact, active FROM drivers ORDER BY name'
    );
    res.json({ success: true, drivers: result.rows });
  } catch (err) {
